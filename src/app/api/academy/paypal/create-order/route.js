...
export async function POST(request) {
  try {
    ...
    const paypalResponse = await fetch(
      `${PAYPAL_API_BASE}/v2/checkout/orders`,
      {
        ...
        headers: {
          ...
          "PayPal-Request-Id": `academy-${checkoutReference}`,
        },
        body: JSON.stringify({
          intent: "CAPTURE",
          purchase_units: [
    ...
    if (!paypalData.id || !approvalLink?.href) {
      return NextResponse.json(
        {
          error:
            "PayPal did not return an order ID and approval URL.",
        },
        {
          status: 502,
        }
    ...
    const { data: payment, error: paymentError } =
      await supabaseAdmin
        ...
        .insert({
          ...
          status: "pending",
          provider_reference: paypalData.id,
          proof_url: null,
          notes: [
    ...
    console.error("ACADEMY CREATE ORDER ERROR:", error);
